...
const buildRegionComboboxOptions = (regions: string[]): ComboboxOption[] =>
  regions.map((region) => ({
    value: region,
    searchValue: region,
    label: (
...
const RegionsSingle = () => {
  ...
  const defaultRegions = useMemo(() => Object.keys(regionConfig), [regionConfig]);
  ...
  const unselectedRegions = allRegions.filter((r) => !currentRegions.includes(r));
...
const RegionsDualInner = ({ production, preview }: RegionsDualInnerProps) => {
  const defaultProdRegions = useMemo(
    () => Object.keys(production.regionConfig),
    [production.regionConfig],
  );
  const defaultPreviewRegions = useMemo(
    () => Object.keys(preview.regionConfig),
    [preview.regionConfig],
  );
  ...
  const allRegions = availableRegions ?? [];
  const unselectedProdRegions = allRegions.filter((r) => !currentProdRegions.includes(r));
  const unselectedPreviewRegions = allRegions.filter((r) => !currentPreviewRegions.includes(r));
  ...
  const onSubmit = async (values: RegionsDualFormValues) => {
    ...
    if (prodChanged) {
      collection.environmentSettings.update(production.environmentId, (draft) => {
        const newConfig: Record<string, number> = {};
        const defaultCount = Object.values(draft.regionConfig)[0] ?? 1;
        for (const region of values.productionRegions) {
          newConfig[region] = draft.regionConfig[region] ?? defaultCount;
        }
        draft.regionConfig = newConfig;
      });
    ...
    if (prevChanged) {
      collection.environmentSettings.update(preview.environmentId, (draft) => {
        const newConfig: Record<string, number> = {};
        const defaultCount = Object.values(draft.regionConfig)[0] ?? 1;
        for (const region of values.previewRegions) {
          newConfig[region] = draft.regionConfig[region] ?? defaultCount;
        }
        draft.regionConfig = newConfig;
      });
