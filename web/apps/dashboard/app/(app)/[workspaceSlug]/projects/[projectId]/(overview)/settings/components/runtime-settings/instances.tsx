...
const InstancesSingle = () => {
  ...
  const defaultInstances = Object.values(settings.regionConfig)[0] ?? 1;
...
const InstancesDualInner = ({ production, preview }: InstancesDualInnerProps) => {
  const prodRegions = Object.keys(production.regionConfig);
  const previewRegions = Object.keys(preview.regionConfig);
  const defaultProdInstances = Object.values(production.regionConfig)[0] ?? 1;
  const defaultPreviewInstances = Object.values(preview.regionConfig)[0] ?? 1;
  ...
  const onSubmit = async (values: InstancesDualFormValues) => {
    if (values.productionInstances !== defaultProdInstances) {
      collection.environmentSettings.update(production.environmentId, (draft) => {
        const updated: Record<string, number> = {};
        for (const region of Object.keys(draft.regionConfig)) {
          updated[region] = values.productionInstances;
        }
        draft.regionConfig = updated;
      });
    }
    if (values.previewInstances !== defaultPreviewInstances) {
      collection.environmentSettings.update(preview.environmentId, (draft) => {
        const updated: Record<string, number> = {};
        for (const region of Object.keys(draft.regionConfig)) {
          updated[region] = values.previewInstances;
        }
        draft.regionConfig = updated;
      });
    }
